\documentclass[a4paper, 10pt, fleqn]{article}
\usepackage{custom}
\usepackage{pageformatting}
\usepackage{tikz}
\usepackage{aeguill}
\usepackage{import}
\usepackage{pdfpages}

\usepackage[ngerman]{babel}
%mathe packages
\usepackage{amsmath}

%table packages
\usepackage{booktabs}
\newcommand{\tabitem}{~~\llap{\textbullet}~~}
\usepackage{tablefootnote}

\graphicspath{{sysspec/pdf/}{images/}{uml/}}

% Einrücken zu Beginn von neuem Absatz unterdrücken
\setlength{\parindent}{0pt}

%============PAGE PROPERTIES=============
\newcommand{\revisiondate}{\today}
\newcommand{\documenttitle}{Clock Pendulum Analyzer} % used for title in title and subtitle pages
\newcommand{\authors}{Tobias Kreienbühl \& Daniel Föhn} %used for title page only
\newcommand{\subauthors}{im Auftrag der Hochschule Luzern} %used for title page only
\newcommand{\subtitle}{Projektdokumentation} % used for title and subtitle pages
\newcommand{\documentdesc}{Gesamtprojektdokumentation des \documenttitle}
\newcommand{\dozent}{Josef Bürgler}
\newcommand{\rpi}{Raspberry Pi 3}
\newcommand{\iic}{$I^2C$}
\newcommand{\hwb}{Hardware-Board}
\newcommand{\titleimage}{gesamtansicht.png}

\begin{document}
	\begin{titlepage}
		\titleGM
		\thispagestyle{empty}
	\end{titlepage}
	\input{redlichkeit}
    
	\tableofcontents
	
    %--- Management Summary ---
    \clearpage
    \input{management_summary}
	\clearpage
	\input{introduction.tex}
    
    %--- Start of Vision ---
    \subtitlepage{Vision - Projektauftrag}
    \subimport{sysspec/}{vision.tex}
    
    %--- Start of PMP ---
    \subtitlepage{Projektmanagement}
    \subimport{pmp/}{projekt_organisation}
    \clearpage
    \subimport{pmp/}{projekt_rahmenplan}
    \clearpage
    %\subimport{pmp/}{zeitplanung}
    \clearpage
    \subimport{pmp/}{risiko}
    \clearpage
    \subimport{pmp/}{projekt_unterstutzung}
    \clearpage
    \subimport{pmp/}{testing}
    
    %--- Start of Sys Spec ---
    \subtitlepage{System Spezifikation}
    \subimport{sysspec/}{hardware}
    \clearpage
	\subimport{sysspec/}{systemdesign.tex}
    \clearpage
    \subimport{sysspec/}{interfaces.tex}
    
    %--- Fazit ---
    \clearpage
   	\input{ausblick}
    \input{fazit}
    
    %BIBLIOGRAPHY
    \clearpage
    \nocite{*}
    \bibliography{bibliography}
    \bibliographystyle{apacite}
    
    %--- Start of Lists ---
    \clearpage
    \listoffigures
    \listoftables
    
    %--- Start of Appendix ---
    \clearpage
   	\section*{Anhang A - Sprint Reviews und Planung}
    \subimport{pmp/}{sprintplanning}
    
    \clearpage
    \section*{Anhang B - Arbeitsjournals}
    \input{../arbeitsjournal/RapportDaniel}
    \clearpage
    \input{../arbeitsjournal/RapportTobias}
    
    \clearpage
    \includepdf[pages=1,pagecommand={\section*{Anhang C - Data Sheets}\subsection*{Adafruit Featherwing}\label{app:adafruit}},scale=0.8,frame=true]{adafruit-ultimate-gps-featherwing.pdf}
    \includepdf[pages={2-},pagecommand={},scale=0.8,frame=true]{adafruit-ultimate-gps-featherwing.pdf}
    \includepdf[pages={1},pagecommand={\subsection*{GPS-Einheit}\label{app:gps}},scale=0.8,frame=true]{GlobalTop-FGPMMOPA6H-Datasheet-V0A.pdf}
    \includepdf[pages={2-4},pagecommand={},scale=0.8,frame=true]{GlobalTop-FGPMMOPA6H-Datasheet-V0A.pdf}
    \includepdf[pages={5},pagecommand={\label{app:gps_occuracy}},scale=0.8,frame=true]{GlobalTop-FGPMMOPA6H-Datasheet-V0A.pdf}
    \includepdf[pages={6-},pagecommand={},scale=0.8,frame=true]{GlobalTop-FGPMMOPA6H-Datasheet-V0A.pdf}
    \includepdf[pages={1},pagecommand={\subsection*{Precision RTC Featherwing}\label{app:rtc_featherwing}},scale=0.8,frame=true]{ds3231-precision-rtc-featherwing.pdf}
    \includepdf[pages={2-},pagecommand={},scale=0.8,frame=true]{ds3231-precision-rtc-featherwing.pdf}
    \includepdf[pages={1},pagecommand={\subsection*{RTC Oszillator Kristall}\label{app:rtc_osc}},scale=0.8,frame=true]{DS3231.pdf}
    \includepdf[pages={2-},pagecommand={},scale=0.8,frame=true]{DS3231.pdf}
    \includepdf[pages={1},pagecommand={\subsection*{K20PB}\label{app:K20PB}},scale=0.8,frame=true]{K20PB.pdf}
    \includepdf[pages={2-},pagecommand={},scale=0.8,frame=true]{K20PB.pdf}
    \includepdf[pages={1},pagecommand={\subsection*{Reflexionslichtschranke SFH9201}\label{app:SFH9201}},scale=0.8,frame=true]{Reflexlichtschranke-SFH9201.pdf}
    \includepdf[pages={2-},pagecommand={},scale=0.8,frame=true]{Reflexlichtschranke-SFH9201.pdf}
    \includepdf[pages={1-},pagecommand={\subsection*{Oszillator Kristall NH25M22TA}\label{app:NH25M22TA}},scale=0.8,frame=true]{c_NH25M22TA_e.pdf}
\end{document}
